...
//
// To use it, declare it before you incldue adodb.inc.php. this isnt the right place
// define('ADODB_ASSOC_CASE', 0);

// Expecation is that composer has already requried the adodb libraries
    ...
    foreach ((array)$ona_contexts[$context_name]['databases'] as $db) {
        ...
        for ($a = 1; $a <= 5 and $connected == 0; $a++) {
            ...
            if (!$ok1 or !$ok2 or $ok3)
                printmsg("{$self['db_type']} DB connection failed: " . $object->ErrorMsg(), 'alert');
...
function db_insert_record($dbh=0, $table="", $insert="") {
    ...
    if ( (!$dbh) or (!$dbh->IsConnected()) or
         (!$table) or
         (!$insert) ) {
        $self['error'] = "Received invalid input";
    ...
    // Run the SQL
    printmsg("Running query: $q", 'debug');
    $ok = $dbh->Execute($q);
    $error = $dbh->ErrorMsg();
    ...
    printmsg("Insert was successful", 'debug');
    return(array(0, 1));
}
...
function db_update_record($dbh=0, $table="", $where="", $insert="") {
    ...
    if ($rs === false) {
        $self['error'] = 'SQL query failed: ' . $dbh->ErrorMsg();
        printmsg($self['error'], 'error');
        return(array(2, 0));
    }
...
function db_delete_records($dbh=0, $table="", $where="") {
    ...
    // Return Success
    printmsg("Query deleted {$rows} row(s)", 'debug');
    return(array(0, $rows));
}
...
function db_get_record($dbh=0, $table="", $where="", $order="") {
    ...
    if (@$self['cache']["db_get_{$table}_record"]['q'] != $q) {
        ...
        printmsg("Row offset reset for table: {$table}", 'debug');
    }
    ...
    // It's the same query again, decide if we should enable adodb disk cache
        ...
        if (!isset($self['db_get_record']['min_rows_to_cache'])) {
    ...
    // Select the record from the DB, cache results
        ...
        printmsg("Running (cached) query: {$q}", 'debug');
    ...
    if (!$rows) {
        ...
        printmsg("Query returned no results", 'debug');
