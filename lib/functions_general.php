...
//   $level = The RFC 5424 log level to use
//
...
*/
    ...
    // expect a string level, if not force alert so we can fix them
    // TODO this can go away when its all clear
    if (is_numeric($level)) {
      $level = 'alert';
      $msg = '--CONVERT LEVEL--: '.$msg;
    }

    // Make sure level is lower case so it can be used as function below
...
// Thanks php for not trusting sys admins to set the clock on their servers.
